...
varying float uClipDist;       // 用于判断裁剪面一边或者另一边
...
void pointLight(in vec3 normal, inout vec4 outAmbient, inout vec4 outDiffuse, inout vec4 outSpecular,
   in vec3 lightPosition, in vec4 inAmbient, in vec4 inDiffuse, in vec4 inSpecular, in int isDirect) {
    ...
    vec3 vp;
    if (isDirect == 1) {
        vp = normalize(lightPosition - (uMMatrix * vec4(aPosition, 1)).xyz);    // 计算从表面点到光源的向量
    } else {
        vp = normalize(lightPosition);
    }
    //vp = normalize(vp);
...
void main() {
    ...
    pointLight(normalize(aNormal), ambientTemp, diffuseTemp, specularTemp,
        uLightPosition, uAmbient, uDiffuse, uSpecular, uIsDirect);
    ...
    uClipDist = dot(aPosition.xyz, uClipPlane.xyz) + uClipPlane.w;
}