...
public class ClipPolyhetron {

    private final static String TAG = ClipPolyhetron.class.getSimpleName();
    ...
    private int uClipPlaneHandle;

    private Context context;
    private boolean isPointLight = true;    // 默认是点光源

    public ClipPolyhetron(Context context) {
        this.context = context;
        init();
    }

    public ClipPolyhetron(Context context, boolean isPointLight) {
        this.context = context;
    ...
    private void init() {
        ...
        mProgram = ShaderUtil.createProgram(ShaderUtil.loadFromAssetsFile("opengles/code/vertex_clip.sh", context.getResources()),
                ShaderUtil.loadFromAssetsFile("opengles/code/fragment_clip.sh", context.getResources()));
        ...
        uClipPlaneHandle = GLES20.glGetUniformLocation(mProgram, "uClipPlane");

        GLES20.glUseProgram(mProgram);
        setPointLight(this.isPointLight);
    }

    public void drawSelf(float[] clipParams) {
        GLES20.glUseProgram(mProgram);
        ...
        GLES20.glUniform4fv(uClipPlaneHandle, 1, BufferUtil.toFloatBuffer(clipParams));
